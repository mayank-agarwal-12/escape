...
@section('content')
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">View Comparisons</h1>
            </div>
            <!-- /.col-lg-12 -->
        </div>
        <!-- /.row -->
                    ...
                    <div class="panel-body">
                        <div class="dataTable_wrapper">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <tbody>
                                @if($comparisonObj)
                                    @foreach($comparisonObj as $comparison)
                                <tr>
                                    <td>{{$comparison->id}}</td>
                                    @if($comparison->tags->count())
                                    ...
                                    <td>{{$comparison->upload->url}}</td>
                                    @if($comparison->tags->count())
                                        <td>Generated</td>
                                        @else
                                    <td> {!! Form::open(['method'=>'PUT' , 'action'=>['Adminpanel\Comparison@update',$comparison->id]]) !!}
                                        {{csrf_field()}}

                                        <div class="form-group col-md-12  ">

                                            {!! Form::submit('Generate Tags',['class'=>'btn btn-primary col-md-12']) !!}
                                        </div>
...
@endif
                                    <td> {!! Form::open(['method'=>'DELETE' , 'action'=>['Adminpanel\Comparison@destroy',$comparison->id]]) !!}
                                        {{csrf_field()}}
