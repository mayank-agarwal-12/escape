<!-- FOOTER -->
<div class="container-fluid nav-default navbar-fixed-bottom1">

<div class="row ">

   {{-- <div class="nav-default navbar-fixed-bottom panel-footer">
        <p class="float-xs-right"><a href="#">Back to top</a></p>
        <p>&copy; <a href="/about">2016 Company,The Instreview</a>  &middot;&nbsp <a href="/contact">Contact Us</a> &middot; <a href="/disclaimer">Terms</a></p>
    </div>--}}
<div class="col-md-12  col-sm-12 col-xs-12 panel panel-default">

</div>

      {{--<div class="col-md-3  col-sm-4 col-xs-4 col-md-offset-2">
      <ul>
          <li >
              <a class="nav-link" href="/">Home
              </a>
          </li>
          <li >
              <a class="nav-link" href="/reviews">Reviews</a>
          </li>
          <li >
              <a class="nav-link" href="/questions">Ask an Expert</a>
          </li>
          <li >
              <a class="nav-link" href="/comparisons">Comparisons</a>
          </li>
          <li >
              <a class="nav-link" href="/applicationhelper">Application Helper</a>
          </li>
          </ul>

</div>

    <div class="col-md-3  col-sm-4 col-xs-4 ">
        <ul>
            <li >
                <a class="nav-link" href="/about">About</a>
            </li>
            <li >
                <a class="nav-link" href="/contact">Contact Us</a>
            </li>
            <li >
                <a class="nav-link" href="/disclaimer">Terms</a>
            </li>
        </ul>

    </div>
    <div class="col-xs-2 col-md-2 col-sm-2 col-lg-2">



        --}}{{--<a class="navbar-brand1" href="/">
            <img alt="Brand" src="/images/logo-2.jpg" height="110px" width="auto">
        </a>--}}{{--
    </div>--}}


      <div class="col-md-3 col-md-offset-1 col-sm-12 col-xs-12">
            &middot; <a href="/about">About</a>&middot;
        </div>
        <div class="col-md-3 col-md-offset-1 col-sm-12 col-xs-12">
            &middot;<a href="/contact">Contact Us</a>&middot;
        </div>
        <div class="col-md-3 col-md-offset-1 col-sm-12 col-xs-12">
            &middot;<a href="/disclaimer">Terms</a>&middot;
        </div>
</div>
    <div class=" text-center">@2016. Instreview Company</div>
    </div>


